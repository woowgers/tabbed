...
static Bool running = True, nextfocus, doinitspawn = True, fillagain = False, closelastclient = False,
...
ssize_t execscript(const char * command, const char * format, ...)
{
  va_list args;
  FILE * fp;
  ssize_t n_reads;
  ...
  n_reads = vfscanf(fp, format, args);
  va_end(args);
  pclose(fp);
...
pid_t gettmuxshellpidbyclientpid(pid_t client_pid)
{
  const char * command_format = ( //
    "tmux list-clients -F '#{client_pid} #{client_session}'"
    "| grep '%d'"
    "| cut -d ' ' -f 2"
    "| xargs -I SAS tmux display-message -p -t SAS '#{pane_pid}'" //
  );
	FILE * fp;
	char command[1024];
	pid_t shell_pid;
...
void propertynotify(const XEvent * e)
{
  ...
  else if (ev->state == PropertyNewValue && ev->atom == XA_WM_HINTS && (c = getclient(ev->window)) > -1 &&
...
void setup(void)
{
  ...
  attrs.event_mask = FocusChangeMask | KeyPressMask | ExposureMask | VisibilityChangeMask | StructureNotifyMask |
                     ButtonMotionMask | ButtonPressMask | ButtonReleaseMask;
  attrs.background_pixmap = None;
  attrs.colormap = cmap;

  win = XCreateWindow(dpy, root, wx, wy, ww, wh, 0, 32, InputOutput, visual,
                      CWBackPixmap | CWBorderPixel | CWBitGravity | CWEventMask | CWColormap, &attrs);
  ...
  XSelectInput(dpy, win,
               SubstructureNotifyMask | FocusChangeMask | ButtonPressMask | ExposureMask | KeyPressMask |
...
void spawn(const Arg * arg)
{
  ...
  pid_t pid, client_pid, shell_pid;
  int status;
  ...
  if (pid == 0)
  {
    ...
    setsid();
    if (sel >= 0 && clients[sel] && clients[sel]->pid)
    {
      ...
      snprintf(name_filename, 32, "/proc/%d/status", shell_pid);
